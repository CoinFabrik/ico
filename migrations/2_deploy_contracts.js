...
module.exports = function(deployer, network, accounts) {
    ...
    if (network != "liveNet") {
        ...
        .then(async function() {
            await deployer.deploy(Crowdsale, MultiSigWallet.address, config.tests.startTime, config.tests.endTime, accounts[0], {gas: 4200000});
        });
    }
    else {
        // !! In production deployment we use config.MW_address as the address of the multisig wallet
        deployer.deploy(Crowdsale, config.MW_address, config.startTime, config.endTime, accounts[0], {gas: 4200000});
    }
};