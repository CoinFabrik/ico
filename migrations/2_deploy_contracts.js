...
module.exports = function(deployer, network, accounts) {
  ...
  if (network != "liveNet") {
    ...
    .then(async function() {
      await deployer.deploy(Crowdsale, config.ether_in_eur, MultiSigWallet.address, config.startTime, config.endTime, accounts[1], config.tranches, {gas: 4550000});
    });
  }
  else {
    // !! In production deployment we use config.MW_address as the address of the multisig wallet
    deployer.deploy(Crowdsale, config.ether_in_eur, config.MW_address, config.startTime, config.endTime, accounts[1], config.tranches, {gas: 4550000});
  }
};
